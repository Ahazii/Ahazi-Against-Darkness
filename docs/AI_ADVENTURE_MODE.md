...
- Prompt builder attaches exported allowlists (monsters, tiles, equipment, traps, events, quest keys, environments).
- Validator **rejects** unknown references with actionable error messages.
...
2. **Allowlists** from `data/adventures/allowlists.json` (generated by `tools/export_adventure_allowlists.py`)
3. Minimal **example adventure** (`data/adventures/examples/`)
4. Instruction: **Return only valid JSON. No markdown fences. No commentary.**
...
For each reference field, value must exist in generated allowlists:

- `rooms[].tile_key` → `tiles.json` keys (`01`–`06`, `11`–`66`)
- `rooms[].encounter.foes[].name` → monster spawn names from `monsters.json`
- `rooms[].trap.key` → trap table keys for environment
- `rooms[].special_event.key` → event table keys
- `treasure.items[]` → equipment / treasure item names
- `quest.complete_when` parameters → valid quest completion types

Unknown references = **hard error** on import.
...
**Tool (planned):** `tools/export_adventure_allowlists.py`  
**Output:** `data/adventures/allowlists.json`
...
| `equipment_items` | `data/rules/equipment_shop.json` |
| `trap_keys` | per-environment trap tables in `dungeon_tables.json` |
| `special_event_keys` | special event/feature tables |
| `quest_keys` | quest table keys |
| `environments` | `dungeon`, `caverns`, `fungal_grottoes` |

Regenerate when rules data changes. CI should fail if allowlists are stale relative to rules (optional check).
...
| Allowlists | `src/app/engine/adventure_allowlists.py`, `tools/export_adventure_allowlists.py`, `data/adventures/allowlists.json` |
