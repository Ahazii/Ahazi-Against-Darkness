...
- **PDF adventure source scanner pass (2026-07-03):** Adventure Management now has **Scan new PDFs** for owned source PDFs in `DATA_DIR/Adventure PDFs` and the legacy `Adventures/` folder. The scanner records local metadata in `DATA_DIR/adventure_pdf_sources.json`: title guess, page count, encryption/text-extraction status, likely module type, confidence, warnings, and recommended conversion path. Source PDFs remain non-playable until a reviewed `adventure.json` manifest is created/imported, so odd modules such as collections, campaign books, programmed/random hybrids, and hex crawls are not forced into the wrong importer. Tables and Rules Reference now include `adventure_pdf_source_scan_table`.
