...
- **Supplement activation preference slice (2026-07-07):** Settings now stores a validated `enabled_supplement_ids` preference list in `game.db` under the user-facing data path. Expanded Edition is always locked on, optional supplement IDs are validated against the registry, and the Settings page explains that these switches are preference-only until session creation consumes supplement activation directly. Tables and Rules Reference document the new preference boundary.
