...
- **Adventure package map-pinning pass (2026-07-04):** Adventure Management -> Modules now creates/refreshes local package records from scanned PDFs, extracts embedded map images into `DATA_DIR/assets/adventures/<package_id>/maps/` when possible, creates a manual map slot when extraction finds nothing, and provides a click-to-fill pin editor for room/scene/hex/location coordinates. Packages are stored in `DATA_DIR/Adventure Packages/<package_id>/package.json`, preserve existing pins on refresh, and remain non-playable review data until a validated manifest uses them. Tables and Rules Reference now include `adventure_package_map_pinning_table`.
