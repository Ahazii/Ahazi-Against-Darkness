...
- **Generated Adventures Guild diagnostics / manual fallback pass (2026-07-02):** Generated Adventures Guild sessions now include backend diagnostics for prompt coverage, current-room action metadata, local narrative extraction state, scene-branch counts, missing prompt errors, and missing scene targets. Resumed generated sessions refresh compatible narrative/prompt metadata on load. Adventure View now exposes **Diagnostics** and **Copy Playtest Report** for generated modules, while **Advanced / Manual Actions** is hidden from normal play and appears only when diagnostics say a missing prompt, missing branch target, or manual-only action needs the fallback.
