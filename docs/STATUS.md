...
- **New home dashboard / TAG recovery pass (2026-06-29):** The existing home screen remains the default, with a compact topbar icon opening a new organized dashboard for Character, Troupe, Guild, Party, Equipment, Finance, Settlement, Campaign, AI Adventure, Rules, Guides, and Developer sections. Live dashboard actions reuse existing dialogs where possible and add direct TAG controls for hiding treasure, rolling hidden-trove risk, recovering a stolen hidden trove, saving Guild state, charging Guild upkeep, and recovering bank robbery by spending 3 Clues and creating the Bandit Hideout lead. Large future surfaces are visibly marked in progress.
...
- TAG remaining full adventure content expansion for deeper automatic scene insertion/removal, exact PDF combat modifiers for Guild spell markers, and broader playtest sign-off;
