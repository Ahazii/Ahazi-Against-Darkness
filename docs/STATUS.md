...
- **Local TAG narrative override pass (2026-07-02):** Generated The Adventures Guild modules can now read `DATA_DIR/tag_scene_narrative_overrides.json` beside `game.db` for user-editable room titles, objectives, logs, and scene descriptions. The committed app seeds a non-overwriting template only; exact PDF prose stays in the user's local data folder rather than the repository. Developer now includes Rules PDF Import: upload an owned rules PDF into `DATA_DIR/rules`, then extract supported Tales from The Adventures Guild Rumor/Scene prose into the local override file. Rumor 3 / Scene 11 has been locally seeded in `.data` for development playtesting so new Paladin's Sword modules can present the old miller's farm setup and no-well/no-sword ending as narrative instead of exposing internal `Lead Trail` / `Complication` scaffolding. Ongoing Quest cards now stop telling generated The Adventures Guild leads to claim a core Epic Reward when that reward path is intentionally blocked.
