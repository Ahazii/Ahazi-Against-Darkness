...
- **PDF Import Review Workspace (2026-07-04):** PDF Module Importer now has a structured review workspace for local `package.json` data. Users can load/edit package details, reviewed pages, rights notes, review status, reviewer notes, room/scene/location/hex nodes, branch targets, local foes/classes/items/tables/trackers/procedures, and see diagnostics before conversion to playable `adventure.json`. Added `docs/ADVENTURE_MODULE_FORMAT.md` as the user guide to module folders, `adventure.json`, `package.json`, maps, pins, procedures, and safe editing.
