...
- **Active registry tooltip slice (2026-07-08):** Adventure View status chips now merge their existing hover text with matching State Registry metadata when a legacy status string maps to a registry row. The Play Context terrain line now exposes matching Terrain Registry metadata. This is display context only; current PDF-backed helpers still apply states, terrain routing, and outcomes.
