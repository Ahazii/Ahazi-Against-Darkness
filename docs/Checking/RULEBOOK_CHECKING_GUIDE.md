...
| TAG.15 | Contextual generated-adventure prompts | TAG scene pages | Start a newly generated TAG module, then inspect Narrative, Current Objective, Ongoing Quest, and TAG Actions during exploration | A **TAG scene prompt** appears for generated TAG rooms. TAG pp.22-24 generated Rumors share horizontal **Investigate** / **Return to town** buttons immediately beneath Narrative; Investigate enters the configured numbered Scene, while Return to town retains the Rumor for later. The choices remain visible when Objective Details is hidden and exclude Director/lifecycle/playbook prose. Later side clue, complication, final scene, and unlocked-scene rooms may use Current Objective and read `source.parameters.tag_reference.room_prompts` for focused shortcuts. Typed procedures replace manual outcome controls where implemented: Mutant Fish Scene 12 shows party Saves, rescuer/victim selectors, and Keep/Sell reward choices in Narrative while persisting every turn. The generated TAG **Director** names the current phase in diagnostic/guidance surfaces, links to Rules Reference/Tables, and explains what kind of decision matters now without replacing player narrative. TAG Actions shows a **Relevant Now** shortcut panel above collapsible Advanced TAG controls with a recommended action, explanation, and focused current action family first. Generated TAG Ongoing Quest cards show a five-step closeout wizard for objective, route/reward, XP, Guild/banking/guidance, and signoff. Older modules automatically repair shared Rumor entry actions and can use Repair guidance for other missing prompt metadata or legacy log wording. Buttons prefill the matching branch/route/reward/XP reference; known profile Clue gates such as Dragon's Lair, Shaura, Daroc's familiar, and Fiendish Abyss prefill the 2-Clue cost. |
