...
| TAG.15 | Contextual generated-adventure prompts | TAG pp.22-31 and relevant scene pages | Start each generated Rumor, leave Objective Details closed, then inspect Narrative, Relevant Now, room metadata, and TAG Actions | Every generated Rumor shows horizontal **Investigate** / **Not now — return to town** buttons immediately beneath Narrative. Investigate enters `tag_reference.entry_scene`; Not now retains the Rumor for later. Verify the choice for both `tag_*` and manifest-owned `imported_*` quest subtypes, especially Rumor 4's `imported_room`. Narrative objective, Relevant Now, and room metadata must call the same special-action dispatcher so Bofto, Medusa, Leprechaun, Mutant Fish, Daroc, Deoldyn, and Shinta controls do not drift between surfaces. The shared host repeats narrative → route → next Scene → typed procedure/combat/vendor/NPC/handoff → reward/completion, while each printed mechanic remains a registered plug-in. Older modules automatically repair the opening and explicit first Scene; a no-scene-graph Rumor 1 must enter Scene 9 rather than Scene 17. Typed procedures replace manual outcome controls where implemented: Mutant Fish Scene 12 shows party Saves, rescuer/victim selectors, and Keep/Sell reward choices in Narrative while persisting every turn. The generated TAG Director and five-step closeout remain diagnostic/guidance surfaces; Advanced/Manual Actions is the fallback. |
