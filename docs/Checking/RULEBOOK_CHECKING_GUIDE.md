...
**Current boundary:** TAG-specific foe names are now spawnable from `tag_monsters.json`, separate from the locked EE bestiary. Major printed rewards, route decisions, XP markers, bank ledgers, route rewrites, Dragon's Lair type reveal module updates, Bandit Hideout stolen-goods room checks, Look Tough, Speedy Recovery, Wizard's Luck, target-specific Guild spell markers, and contextual generated-adventure prompts have UI actions. Remaining TAG work is broader playtest signoff.
