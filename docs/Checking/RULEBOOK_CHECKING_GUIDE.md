...
- **Copy Narrative Report** — exact player Narrative followed by diagnostics; Party finance lines distinguish carried, banked, and total gold
...
| TAG.2 | Medusa rumor exact foe and typed Scene 10/1/6 procedures | TAG pp.6-8, 22, 25-28; EE pp.101, 162 | Rumor Scene Detail `2` | Scene 10 rolls one exploding L6 group Stealth Save with the lowest living-party modifier; this is one party roll, not one roll per hero. Narrative/UI list every hero's modifier and identify the controlling modifier. Failure stores d3+2 HCL+2 dagger agents and presents a labelled L5 Streetwise actor selector or immediate combat with the party acting first. Failed Streetwise gives the agents first action; victory stages 4d6 gp. Final room is Xasartha's cabin; Scene 1 choices appear before combat and Xasartha spawns only when the printed result says combat starts. Reaction `1` persists one `6d6` bribe and offers exact carried-gold, one carried 15gp+ gem/jewel, or refusal; reactions `3-5` fight and `6` suppresses Morale. A persisted reaction `2 quest` presents Accept/Refuse without rerolling; acceptance rolls the EE Quest Table and refusal lets Xasartha leave. An accepted Bring Gold Quest shows its exact stored amount, party total, and Quest-giver turn-in; exact payment clears the core Quest and awards one Epic Reward. The encounter remains peaceful, grants no combat treasure, and offers **Continue — return to town and finish** instead of generic generated-lead guidance. Defeating Xasartha awards the normal major-foe XP roll, persists `2d6` necros, and offers the printed 260gp sell-untried or wear choice. Scene 6's pendant persists and recharges through the existing Luck controls for one point, or two additional points to a halfling; normal magic-item class restrictions apply. |
| TAG.2a | Daroc's Lost Familiar typed Scene 5 | TAG pp.20, 24, and 26; Crucible of Classic Critters pp.11-15 for future class/companion integration | Rumor Scene Detail `9` | Reaching Scene 5 does not complete the quest. The panel shows progress, a searcher selector, a separate living reward-recipient selector, **Search for Clues**, and **Give up — return to town**. Each search automatically spends the selected hero's `d6` bribe and rolls that hero's Streetwise against L6; repeated results and Town Streetwise Clue provenance persist through save/resume. Scene 5 counts only those marked Clues and requires two, or one with a Druid, Beastmaster, cat-like hero, or cat animal companion. Give up ends this session, returns Rumor 9 to `heard`, and preserves eligible Clues. Success spends the exact eligible Clues, gives the selected recipient 200gp, adds one pending XP roll, and resolves once. The 200gp amount follows the p.24 Rumor offer by player ruling over Scene 5's known erroneous 100gp line on p.26; new and resumed runtime narrative must therefore say 200gp. XP assignment/banking remains usable before Continue. Copying the Narrative Report must show carried, banked, and total gold separately and must not mutate finance state. |
| TAG.2b | Generated lead terminal acknowledgement | App-owned presentation after a printed Scene resolves | Complete any typed generated TAG terminal with Objective Details collapsed | Narrative, diagnostics, the compact Narrative-header action, and the prominent primary action must all say **Continue — return to town and finish**. The prominent action cannot be hidden by the optional Objective Details preference; save/resume must preserve the result and normalize older wording without replaying it. |
...
| TAG.10 | Scene result controls | TAG scene/theme pages | TAG Actions → Scene result | Printed results apply and log common rewards such as the Medusa pendant, gargoyle bounty, Gorungar bounty, bandit capture, Shaura reward, Agaratha, and Dragon's Lair type reveal. Mutant Fish Scene 12 and Deoldyn Scene 3 now use dedicated typed generated-Rumor hosts instead of manual Scene result/XP controls; the old Deoldyn selector is legacy/manual diagnostics only. Dragon's Lair reveal also rewrites the latest installed Dragon's Lair finale title/reference while keeping the validated Young Dragon encounter proxy |
...
| TAG.12 | Trinkets and Guild spells | TAG pp.61-66 | TAG Actions → Trinket / Guild spell | Carried trinkets/scrolls are consumed where present; Speedy Recovery marks settlement healing at 2 Life/day; Look Tough is consumed on the next Streetwise roll; Wizard's Luck modifies Gambling House resolution; Temporary Weapon stores its seven-day campaign expiry, works as magic with no Attack bonus, and expires after qualifying magic-only use; Troupe Switch and Silence use target-specific markers |
...
| TAG.15 | Contextual generated-adventure prompts | TAG pp.22-31 and relevant scene pages | Start each generated Rumor, leave Objective Details closed, then inspect Narrative, Relevant Now, room metadata, and TAG Actions | Every generated Rumor shows horizontal **Investigate** / **Not now — return to town** buttons immediately beneath Narrative. Investigate enters `tag_reference.entry_scene`; Not now retains the Rumor for later. Narrative objective, Relevant Now, and room metadata must call the same special-action dispatcher so Bofto, Medusa, Leprechaun, Mutant Fish, Daroc, Deoldyn, and Shinta controls do not drift between surfaces. The shared host repeats narrative → route → next Scene → typed procedure/combat/vendor/NPC/handoff → reward/completion, while each printed mechanic remains a registered plug-in. Older modules automatically repair the opening and explicit first Scene; a no-scene-graph Rumor 1 must enter Scene 9 rather than Scene 17. Required scenes must use `tag_scene_resolved`, never generic arrival completion. Mutant Fish Scene 12 automatically rolls every living hero's Save on entry, persists the individual results, then shows rescuer/victim and Keep/Sell controls; Keep/Sell exposes explicit return-to-town completion. Daroc Scene 5 repeats selected-searcher Streetwise checks until its Clue threshold, or ends the current attempt through non-permanent Give up; neither route can be replaced by arrival completion. Legacy affected saves repair without rerolling. The generated TAG Director and five-step closeout remain diagnostic/guidance surfaces; Advanced/Manual Actions is the fallback. |
| TAG.15d | Rumors 6/11 repeatable services | TAG p.23 and pp.25-26 Scene 2; EE p.76 Scrolls; TAG p.24 and p.26 Scene 3 | Generate fixed Rumor 6 and 11 modules; inspect entry, final Scene, save/resume, both horizontal dividers, independent scrollbars, Done, and closeout | Entry shows only **Investigate** / **Not now — return to town**. Rumor 6 then opens one persisted Blackbird Hill host: exact 200gp Shoes purchases, at most one pair per eligible wearer, party-owned/party-Tier hireling handling, and one automatic lesson for 100gp or automatically free after three pairs. Under the player-confirmed campaign interpretation, every living non-Barbarian is a valid learner; a non-spellcaster retains one use per adventure and casts it at +1. The saved inner Narrative/service split defaults to about 40/60, keeps Narrative at least 96px high, resets on double-click, and gives Narrative and the service independent vertical scrollbars; the outer divider still resizes their combined area against the map. Rumor 11 opens one complete simultaneous batch: living bow-capable trainees only, exact `60gp × Level`, each row shows its own carried + home-bank = available balance, bank-only Level 10 funds remain eligible, payment spends home-bank gold first, whole-batch validation occurs before mutation, every payment precedes every automatic XP roll, payment remains spent on failure, Deadly Accuracy/Dead Shot or normal advancement is available only for the base Elf, no later batch is allowed, and **Done — finish training** resolves the visit. Either service may be skipped, arrival never completes it, resume never duplicates transactions or rolls, and Done reveals the shared Continue closeout. Rumor 6 passed live on 2026-08-04. Rumor 11's opening, entry, transfers, camp return, and route back to Deoldyn are passed; its v0.39.73 gate now checks the visible inner divider with Objective Details collapsed, one bank-funded training batch, Done, and Continue without replaying any passed work. |
| TAG.15e | Rumor 11 current live gate | TAG p.24 Rumor 11; TAG p.26 Scene 3; TAG p.9 bank availability | Resume preserved session `fc741849402d46e096b2efa52368de8f` at Deoldyn; leave **Objective Details** collapsed | The prior return retry and route are passed: the service is open, the training batch is unresolved, and Sir Benedict remains eligible at 0gp carried + 600gp banked. On v0.39.73 the nested divider must remain visible whenever this tall service is visibly rendered, even if Objective Details is collapsed. Drag/reset both dividers, then run only Sir Benedict's bank-funded automatic batch, **Done — finish training**, and the shared Continue closeout. Do not replay the opening, transfers, camp return, route, or Rumor 6. |
...
| TAG.18 | Generated lead structure | TAG pp.22-59 | Generate Rumor, Treasure Map, Thematic Dungeon, and Guild Job leads, especially Rumors 6 and 11 | Each generated module has a `lead_structure` that matches the PDF result: dungeon for true exploration, scene_chain for choose/go-to Scene branches, procedure for compact checks/red herrings/escorts, vendor for purchases/spell lessons, trainer for paid training, and handoff when the PDF points to a separate generated dungeon. The Current Objective buttons should match that structure instead of presenting a fake combat or generic TAG Actions workflow. Blackbird Hill is the persisted vendor/Done reference; Deoldyn is the complete simultaneous trainer-batch/Done reference. Neither may expose its service before the common Rumor opening reaches the printed Scene. Assigned hireling Shoes remain party-owned and accessible: the last backing pair cannot be sold, transferred, or hidden in a Bag/Kukla compartment while the hireling travels with the party, but surplus pairs remain movable. Legacy inventory or durable skills count as this visit only when resolved/reward/closeout evidence exists, never from a stale arrival-completed flag alone. |
| TAG.19 | Shinta / Agaratha solo handoff | TAG Rumor 12, Scene 4, Scene 7 | Generate Rumor Scene Detail `12`, start the module, press **Choose Shinta's champion** | The generated module has no optional side-clue room. Scene 4 lists only one eligible champion: sword/edged-hand-weapon capable, living, not demon, undead, or chaos-tainted. The route log records Scene 7 as a solo ten-room Bandit Hideout handoff: allies may lend items but cannot help, minion counts are reduced by 1 minimum 1, Final Boss uses the Riff-Raff Table without count reduction, and Agaratha is awarded only after the champion defeats the final boss and returns |

**Current boundary:** TAG-specific foe names are now spawnable from `tag_monsters.json`, separate from the locked EE bestiary. Major printed rewards, route decisions, XP markers, bank ledgers, route rewrites, Dragon's Lair type reveal module updates, Bandit Hideout stolen-goods room checks, Ghastly Mine/Fiendish Abyss/Minotaur Maze procedure rolls, Guild Job procedure rolls, Look Tough, Speedy Recovery, Wizard's Luck, automatic Temporary Weapon Enchantment timing, target-specific Guild spell markers, and contextual generated-adventure prompts have UI actions. Remaining TAG work is broader playtest signoff and one source-backed typed procedure at a time.
...
| 12d.7 | Roll Streetwise actions | Look for Clue lets the player choose the character, spends `d6` gp, rolls against L6, and records Town Streetwise Clue provenance on success; Listen to Rumors, Interrogate, and Look Tough log the correct TAG p.20 result class |
