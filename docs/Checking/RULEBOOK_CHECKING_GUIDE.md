...
| TAG.12 | Trinkets and Guild spells | TAG pp.61-65 | TAG Actions → Trinket / Guild spell | Carried trinkets/scrolls are consumed where present; Speedy Recovery heals to full Life; Look Tough, Troupe Switch, Silence, Wizard's Luck, and weapon enchantment markers are applied and logged |
...
**Current boundary:** TAG-specific foe names are now spawnable from `tag_monsters.json`, separate from the locked EE bestiary. Major printed rewards, route decisions, XP markers, and bank ledgers have UI actions. Remaining TAG work is to let route markers rewrite or expand generated modules automatically and to turn Guild spell markers into exact combat/exploration modifiers where the PDF calls for them.
