...
| TAG.15d | Rumors 6/11 repeatable services | TAG p.23 and pp.25-26 Scene 2; EE p.76 Scrolls; TAG p.24 and p.26 Scene 3 | Generate fixed Rumor 6 and 11 modules; inspect entry, final Scene, save/resume, divider resize, Done, and closeout | Entry shows only **Investigate** / **Not now — return to town**. Rumor 6 then opens one persisted Blackbird Hill host: exact 200gp Shoes purchases, at most one pair per eligible wearer, party-owned/party-Tier hireling handling, and one automatic lesson for 100gp or automatically free after three pairs. Under the player-confirmed campaign interpretation, every living non-Barbarian is a valid learner; a non-spellcaster retains one use per adventure and casts it at +1. The guided service remains vertically scrollable when constrained, so its fields and profile-specific Done stay reachable, while the user's horizontal Narrative/map divider height remains authoritative. Rumor 11 opens one complete simultaneous batch: living bow-capable trainees only, exact `60gp × Level`, whole-batch validation before mutation, every payment before every automatic XP roll, payment retained on failure, Deadly Accuracy/Dead Shot or normal advancement only for the base Elf, no later batch, and **Done — finish training**. Either service may be skipped, arrival never completes it, resume never duplicates transactions or rolls, and Done reveals the shared Continue closeout. Rumor 6 passed live on 2026-08-04; resume the preserved Rumor 11 session after the divider correction rather than replaying its passed opening. |
