...
| TAG.10 | Scene result controls | TAG scene/theme pages | TAG Actions → Scene result | Printed results apply and log: Medusa pendant, gargoyle bounty, Gorungar bounty, bandit capture, Shaura reward, Daroc's cat, Agaratha, Deoldyn training, and Dragon's Lair type reveal. Mutant Fish Scene 12 now uses its dedicated typed Narrative procedure instead of manual Scene result/XP controls. Deoldyn Scene 3 should behave as a trainer result, not a dungeon fight: only bow-capable living characters are offered, the player chooses Deadly Accuracy or Dead Shot, the app charges 60 gp x Level, rolls the printed XP check, and records the selected skill only on success. Dragon's Lair reveal also rewrites the latest installed Dragon's Lair finale title/reference while keeping the validated Young Dragon encounter proxy |
...
| TAG.18 | Generated lead structure | TAG pp.22-59 | Generate Rumor, Treasure Map, Thematic Dungeon, and Guild Job leads, especially Rumor 11 | Each generated module has a `lead_structure` that matches the PDF result: dungeon for true exploration, scene_chain for choose/go-to Scene branches, procedure for compact checks/red herrings/escorts, vendor for purchases/spell lessons, trainer for paid training, and handoff when the PDF points to a separate generated dungeon. The Current Objective buttons should match that structure instead of presenting a fake combat or generic TAG Actions workflow |
