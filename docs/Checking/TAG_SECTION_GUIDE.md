...
- **Scene result** applies common printed rewards such as the Medusa pendant, gargoyle bounty, Gorungar rewards, bandit capture, Shaura reward, mutant-fish rations, Agaratha, Deoldyn training, and Dragon's Lair reveal. Daroc's cat uses its dedicated typed Scene 5 control instead of this generic result.
...
Daroc's Lost Familiar uses TAG p.27, Scene 5 directly. Successful settlement **Look for Clues** actions mark their Clues as town-generated. The scene counts only those markers, reduces the cost from two to one for a Druid, Beastmaster, cat-like hero, or cat animal companion, asks who receives Daroc's 100gp, spends the eligible Clues, and adds one pending XP roll. Beastmaster and companion systems themselves remain governed by the future Crucible of Classic Critters pp.11-15 implementation.
