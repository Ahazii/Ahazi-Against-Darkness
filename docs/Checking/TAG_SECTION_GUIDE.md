...
- **Guild spell** consumes scrolls when present, logs known-spell casts, and applies safe markers. Speedy Recovery marks settlement healing at 2 Life per day, Look Tough is consumed on the next Streetwise roll, Wizard's Luck modifies the Gambling House workflow, and the optional spell target fields record the weapon/recipient/Stealth-swap target for Temporary Weapon Enchantment, Troupe Switch, and Silence of the Mouse. Temporary Weapon Enchantment also exposes the TAG p.65 player choice when Invisible Gremlins, revealed Gremlins, or an Iron Eater could take or destroy the weapon.
