...
| Combat modifiers | starter | Blade poison, poison foes, two-step MR (connect + penetrate tiers), mirror-image absorption, subdual, bribes, missiles, weapon-type modifiers; troll regeneration (fire, acid vials, lightning, lantern oil suppress regen), held/fog/specter combat effects; post-round log summary with party wounds called out; multi-target spell/ability UI (Double Attack, Double Kick, Protective Incense, Infallible L8+). |
...
| Equipment shop | starter | `equipment_shop.json` + home UI; p.16 buy / p.19 sell; class restrictions; active camped heroes can spend carried + banked gold; lantern oil and acid vials; Big Money Buyer triples one gem/jewel sale. |
...
| Combat Focus | starter | Tactical map, top foe chip strip (category colors, grouped minor foes, Final Boss emphasis, hover/click foe traits), command rail (Exits/Encounter/Log), hero drawer (expert + heroic ability picks), slim deck, cinema view; one-viewport layout. |
