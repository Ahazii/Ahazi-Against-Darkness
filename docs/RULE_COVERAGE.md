...
| Special events | starter | Ghost, trap, healer (once per adventure reroll), alchemist, Lady in White, wandering subtable; caverns/fungal environment tables. Event/feature table results now log visible `Event:` / `Feature:` lines even in Summary mode. State changes and damage from special events/features log targeted `Effect:` lines, including the affected hero. |
