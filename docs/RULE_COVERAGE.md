...
| Reactions and morale | starter | Per-foe bestiary tables + bribes for all current indexed spawn names; p.146 immediate action vs Check Reactions (mutually exclusive); category fallback for mixed/unknown groups. **Capture reaction (p.102) wired 2026-06-05; equipment strip/restore completed 2026-06-13**: minions may roll "capture" (roll=1); foes attack non-lethally; heroes reaching 0 Life are taken prisoner (one foe escorts each captive); stripped gold drops as treasure and stripped equipment is restored on rescue/ransom. Magic Challenge reaction rows now resolve as magical Saves: success ends peacefully, failure gives foes first strike. |
...
| Combat Focus | starter | Tactical map, top foe chip strip (category colors, grouped minor foes, Final Boss emphasis, hover/click foe traits/statuses), command rail (Exits/Encounter/Log), hero drawer (expert + heroic ability picks), slim deck, cinema view; one-viewport layout. Final Boss defeat shows a clear completion banner before the party exits. |
...
| Capture reaction | starter | EE p.102: **wired 2026-06-05; equipment strip/restore completed 2026-06-13**. Minion foes may roll Capture (d6=1 on minion reaction table); attacks become non-lethal; heroes reaching 0 Life are captured (one foe escort per captive, stripped of gold and carried equipment). Spend 3 Clues on "Someone Has Been Imprisoned" to locate and generate the hideout tile (doubled guards, 2d6×2d6 cave adjacent to origin). Rescue by winning the hideout fight (captives revive with d3 Life and recover stripped equipment) or paying a ransom (Level×10 gp each). Captured hero status shown prominently in the party sheet with contextual Find-Hideout and Pay-Ransom buttons. Tests: `tests/test_capture.py`. See gap table below for deferred items (ranger Clue reduction). |
