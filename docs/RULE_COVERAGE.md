...
| Combat Focus | starter | Tactical map, top foe chip strip (category colors, grouped minor foes, Final Boss emphasis, hover/click foe traits/statuses), command rail (Exits/Encounter/Log), hero drawer (expert + heroic ability picks), slim deck, cinema view; one-viewport layout. Summary log mode keeps outcome summaries plus state/effect changes while hiding roll/math detail. Final Boss defeat shows a clear completion banner before the party exits. |
