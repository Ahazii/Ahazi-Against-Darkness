...
| Adventurers' Guild | partial campaign | Persistent roster, multiple parties, banking, **campaign state** (`days_passed`, TAG banking toggle, `/api/campaign`), home settlement identity/size/notes, troupe/guild/coffer state, active party selection, settlement storage, magic lockers, platinum pieces, special-item availability, Streetwise Look for Clues/rumors/interrogation/Look Tough, travel to new settlements with simple or hex-route logging, treasure-map follow rolls, Gambling House gp outcomes, first twenty-eight treasure/service/procedure rows from TAG pp.9-20, p.54, and pp.61-65, TAG special foe supplement, TAG lead generation into normal installed adventures, and TAG Actions for branch/clue/count/capture/reward/trinket/Guild-spell/finance handling | Deeper per-scene branch-specific reward automation, full Guild spell combat/exploration math, richer per-character bank account ledgers |
...
- TAG Actions section resolves/logs social branches, Clue spends, variable counts, capture-alive outcomes, printed rewards, trinket use, Guild spell use, inheritance notes, storage robbery risk/recovery, moneylender enforcement, and guild upkeep with hover hints.
...
- deeper per-scene reward automation and exact branch-specific item/XP handling;
- full Guild spell combat/exploration math beyond current status/log markers.
