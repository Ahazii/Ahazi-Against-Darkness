...
- TAG Actions section resolves/logs social branches, Clue spends, variable counts, capture-alive outcomes, printed rewards, exact scene rewards, structured route markers, safe route rewrites to the latest generated module, structured XP markers/awards, trinket use, Guild spell use/marker clearing, per-character bank deposit/withdrawal, inheritance notes/transfers, storage robbery risk/recovery, moneylender enforcement, and guild upkeep with hover hints.
...
- deeper automatic scene insertion/removal from the structured route markers;
