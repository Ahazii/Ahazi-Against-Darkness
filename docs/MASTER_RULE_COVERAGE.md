...
Recent coverage promoted:

- FD p.58 traps resolve with FD-specific Save math, class/armor modifiers, Halfling rerolls, Magic Resistant Liquid lockout, Beast Cage spawns, Soulbinding choices, and Disintegration item-sacrifice/incineration choices.
- FD p.62 treasure choices remain app-driven through the room treasure prompt and Current Objective signoff.
- FD p.63 Winds of Despair now pauses for each living hero's printed 1 Madness or 2 Life choice.

Remaining content families include room content sign-off, Citadels, rare event edges,
and higher-tier Secrets.
...
- Daroc's Lost Familiar (TAG pp.20, 24, and 26) with actor-selected repeatable L6 Streetwise searches and `d6` bribes, persisted Town Streetwise Clue provenance/progress, two-Clue cost, one-Clue Druid/Beastmaster/cat eligibility, a separate selected living 200gp recipient, and one pending XP roll. The 200gp amount follows the Rumor offer on p.24 by player ruling over Scene 5's known erroneous 100gp line on p.26. New/resumed runtime narrative uses 200gp consistently, and Narrative Debug Reports distinguish carried, banked, and total gold without changing finance state. Give up returns Rumor 9 to `heard` and preserves eligible Clues; required scene completion prevents arrival or duplicate rewards. Full Beastmaster and companion behavior remains deferred to Crucible of Classic Critters pp.11-15;
...
- Rumor 4's TAG p.29 Scene 12 action is registered as automatic on entry and required for completion. The app makes one persisted L5 hypnosis Save for every living hero without a player roll button, prevents generic arrival completion, owns rescue/reward/XP through Keep/Sell, and then exposes the explicit return-to-town finish action. Legacy arrival-completed saves repair idempotently.
- Rumors 6 and 11 use one persisted service host and one required explicit Done lifecycle instead of completing on arrival. Rumor 6's TAG p.23 lead reaches Scene 2 on pp.25-26, where optional 200gp Shoes of Fast Walk purchases may repeat and the one 100gp/free-after-three-pairs illusion lesson remains available until **Done — leave Blackbird Hill**. Per the player-confirmed interpretation, living magic-item-eligible heroes and living hirelings may wear Shoes, animal companions may not, and a hireling wearer uses the party Tier while the pair remains party-owned and returns automatically when that hireling leaves. The lesson's separate player-confirmed interpretation allows any living non-Barbarian to learn automatically as from a scroll; a non-spellcaster retains one use per adventure and casts it at +1 (EE p.76). A saved inner horizontal divider allocates the combined top panel between Narrative and the generated service at a 40/60 default, preserves at least 96px for Narrative, and gives both regions independent vertical scrollbars; the existing outer divider still resizes the combined panel against the map. Rumor 11's TAG p.24 lead reaches Scene 3 on p.26, where the complete simultaneous batch of selected living bow-capable trainees and `60gp × Level` costs is validated against each trainee's own combined carried + home-bank gold. The UI exposes carried, banked, and available totals, payment spends that trainee's home-bank gold first, every payment is taken before automatic XP rolls, failures keep their cost, and only the normal/base Elf may choose ordinary level advancement instead of Deadly Accuracy or Dead Shot. The player may skip training, but no later trainee may be added after the batch is rolled; **Done — finish training** then resolves the visit. Fresh and legacy manifests use `tag_scene_resolved`; resume retains prior service transactions and results. The shared TAG pp.22-24 opening decision is persisted independently of room position: camp re-entry never repeats a resolved Investigate choice, while a fresh start-camped Rumor enters before presenting it. Rumor 6 passed live on 2026-08-04; Rumor 11's opening, Scene 3 entry, camp return, earlier divider correction, and transfers passed, with the bank-display/nested-layout check and paid training remaining on v0.39.72.
- Generated TAG terminal acknowledgement is shared presentation rather than Rumor-specific procedure code: **Continue — return to town and finish** stays visible as a required primary action when Objective Details is collapsed, matches Narrative and diagnostics, and preserves old pending sessions through wording-only resume normalization.
- TAG Actions section resolves/logs social branches, Clue spends, variable counts, capture-alive outcomes, printed rewards, exact scene rewards, structured route markers, safe route rewrites to the latest generated module, Clue-gate follow-up scene insertion, optional side-scene removal, structured XP markers/awards, trinket use, Guild spell use/marker clearing, automatic TAG p.65 Temporary Weapon Enchantment timing, Speedy Recovery settlement-healing markers, Look Tough Streetwise bonus consumption, Wizard's Luck Gambling House resolution, per-character bank deposit/withdrawal, inheritance notes/transfers, storage robbery risk/recovery, bank robbery recovery into Bandit Hideout, moneylender enforcement, guild upkeep, Guild 50% loot share, resurrection funding, and Guild reroll reset with hover hints.
