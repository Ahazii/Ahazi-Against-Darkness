...
| Adventurers' Guild | partial campaign | Persistent roster, multiple parties, banking, **campaign state** (`days_passed`, TAG banking toggle, `/api/campaign`), home settlement identity/size/notes, troupe/guild/coffer state, active party selection, settlement storage, magic lockers, platinum pieces, special-item availability, Streetwise Look for Clues/rumors/interrogation/Look Tough, travel to new settlements with simple or hex-route logging, treasure-map follow rolls, Gambling House gp outcomes, first twenty-eight treasure/service/procedure rows from TAG pp.9-20, p.54, and pp.61-65, TAG special foe supplement, and TAG lead generation into normal installed adventures with structured Rumor/Thematic/Guild Job rule profiles | Fully automated branches for bespoke TAG scenes, trinket-use buttons, Guild spell casting effects, richer bank inheritance/robbery recovery |
...
- fully automated branch choices for bespoke Rumor Scene, Thematic Dungeon, and Guild Job results;
