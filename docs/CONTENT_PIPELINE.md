...
3. Extracted map images are stored in `DATA_DIR/Adventures/<adventure_id>/maps/`. If no embedded PDF image can be extracted, the package creates a manual map slot; place a cropped map image at the displayed `DATA_DIR/Adventures/<adventure_id>/maps/...` path. Use the package map preview to click/fill percent coordinates, tie room/hex/location ids to pins, and set a pin role such as room, dungeon entrance, dungeon exit, stairs, secret, objective, camp, or settlement. Keep the source PDF and source page on every map/pin set so the player can audit it.
