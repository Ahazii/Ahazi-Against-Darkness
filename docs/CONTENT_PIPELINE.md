...
0. Place owned source PDFs in `DATA_DIR/Adventure PDFs` (the legacy repo `Adventures/` folder is still scanned for local development copies), then use Adventure Management -> PDF Module Importer -> **Scan New PDFs**. The scan writes metadata only to `DATA_DIR/adventure_pdf_sources.json`: title guess, page count, encryption/text-extraction status, likely module type, map/pin signals, table/foe/class package signals, confidence, warnings, and recommended conversion path. The importer opens as a module list with status, Create / Refresh, Validate, Edit / Check, and Delete Package controls.
1. Inventory PDF pages, text length, and embedded images.
2. If the PDF adds module-local content, use Adventure Management -> PDF Module Importer -> **Create / Refresh** to create a declarative package matching `data/adventures/schema/adventure_package.v1.json`. Packages live inside the adventure folder at `DATA_DIR/Adventures/<adventure_id>/package.json` and can record reviewed nodes, local tables, foes, items, class candidates, states, rules, trackers, procedures, imported map assets, map pins, and extracted artwork. They cannot execute scripts.
3. Extracted map images are stored in `DATA_DIR/Adventures/<adventure_id>/maps/`. The importer first extracts embedded images; if the PDF does not expose the map as an image, it renders likely map pages, including page 4 for programmed adventure PDFs with map signals, as PNG files. If no map image can be extracted or rendered, the package creates a manual map slot; place a cropped map image at the displayed `DATA_DIR/Adventures/<adventure_id>/maps/...` path. If an existing package still shows only a manual slot after importer changes, use Map Review -> **Re-extract Maps**. Use the package map preview to click/fill percent coordinates, tie room/hex/location ids to pins, and set a pin role such as room, dungeon entrance, dungeon exit, stairs, secret, objective, camp, or settlement. Keep the source PDF and source page on every map/pin set so the player can audit it.
4. Use the PDF Import Review Workspace to extract and inspect candidate lists for locations/nodes, tables, foes, items, classes, states, rules, and procedures. Click any candidate to inspect source page, source text, branches, table rows, procedure steps, and raw JSON. If a candidate is useful but misclassified, move it to the correct list. If it is junk, mark it wrong/ignored so it is preserved under `ignored_records` for future importer improvement.
4a. Use Extracted Artwork Library -> **Extract Artwork Library** to copy every PDF-exposed image into `DATA_DIR/Adventures/<adventure_id>/artwork/extracted/` and register it under `package.json` `artwork[]`. Treat this as a raw local library: useful art, maps, logos, masks, and junk page furniture can all appear. Review and assign images later; do not distribute PDF-derived artwork unless licensed.
5. Edit structured review nodes in the Location Preview / Location Editor: rooms, scenes, locations, hexes, branches, linked foes, linked items, linked procedures, map pins, app notes, and source pages. Use the Imported Record Editor for foes, items, classes, states, rules, tables, trackers, and procedures before falling back to raw JSON. Item records expose description, modifiers, applied states, sale price, sellable, and buyable fields. Keep uncertain text as `needs_pdf_check` until the PDF has been checked.
...
- Use an adventure package when a PDF introduces its own maps, numbered locations, custom roll tables, new foes, new items, new classes, states/conditions, module-local rules, doom/event trackers, or branch procedures that do not fit the base manifest.
