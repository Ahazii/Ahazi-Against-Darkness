...
2. If the PDF adds module-local content, use Adventure Management -> PDF Module Importer -> **Create / Refresh** to create a declarative package matching `data/adventures/schema/adventure_package.v1.json`. Packages live inside the adventure folder at `DATA_DIR/Adventures/<adventure_id>/package.json` and can record reviewed nodes/locations, local tables, foes, items, class candidates, states, rules, trackers, procedures, terrain candidates, room-tile candidates, generator candidates, campaign-state candidates, narrative blocks, imported map assets, map pins, and extracted artwork. They cannot execute scripts.
