...
Last updated: 2026-07-15. Target build: post-v0.39.17 FD spider reaction / Citadel trap follow-up patch.
...
The latest disposable Forsaken Depths Game 2 report was session `f0ab5c80eee34a9c9cec7b3e95484823`. The older live session `fed118f1c70a4835b84f245bf8ddefeb` may now be complete. Use the active disposable FD session if it is still available; otherwise start a fresh disposable FD Game 2 after deployment.
...
2. Force **FD foe encounter -> Vermin -> Spore Spiders**. Confirm the log uses the **Spore Spiders reaction table**, they always fight to the death, and they do not flee on morale. Also note whether the encounter-start 4-in-6 surprise roll appears.
3. Force **FD foe encounter -> Vermin -> Deep Cave Spiders** only if you want one more spider check. Confirm Morale is at `-1`, no treasure is awarded, and if at least two spiders are killed while others remain, the log reduces the remaining spiders by 1 Level per two killed to minimum L3. The printed death-spawn/no-resurrection rule is still a future fidelity hook unless the log explicitly shows it.
4. Continue the Citadel of Traps until a room-content result that would be Minions or Hordes appears. FD p.60 then makes a 4-in-6 replacement check; if it becomes a trap, confirm the room has a concrete FD trap name/level and the **Resolve Trap** action appears. Resolve one such trap and confirm any post-trap treasure can be claimed normally.
5. Move one room farther into the Citadel, save, return to Dashboard, resume, and confirm the same Citadel room restores. Then use the side-sheet return route when available and confirm the Citadel marker clears after returning to the main dungeon map. Legacy saves that already navigated back to the origin tile should also clear the stale Citadel marker after reload/refresh.
6. Force **FD foe encounter -> Boss -> Dark Elf Warlock**. Confirm the defeated Warlock awards one FD treasure roll at `+2`. Treat the printed ice-blast targeting as still under review unless the log explicitly shows the Defense roll: FD p.44 says the Warlock makes one staff attack and one ice blast each turn; shields count, armor is ignored except Cold defense, and barbarians/Ice-based characters add +1/2 L.
