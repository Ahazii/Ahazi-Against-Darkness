...
2. In the resumed Citadel room, confirm the side-sheet exit is clearly labelled **Return to main map**. Use it when safe; on the origin/main-map tile, confirm the route back is labelled **Enter Citadel sheet**. Re-enter and confirm the Citadel marker returns only while actually on the side sheet.
3. If Sly Silas or another hero still has `FD Soulbound:<tile id>`, cast **Blessing** on that hero. Confirm the Narrative logs `Blessing frees <name> from Soulbinding (FD p.58)`, the `FD Soulbound` chip disappears, and any pending Soulbinding Life/Madness prompt is cleared. If a fresh Soulbinding trap is tested instead of curing the existing one, move away once and confirm the **Lose 1 Life** / **Gain 1 Madness** buttons appear before continuing.
...
8. Record as lower-priority visual debt, not a blocker for rules testing: the first forced Citadel side-sheet tile in session `003f51881ebc417a8a70891da9e0de00` was clipped badly. Investigate side-sheet placement/truncation after the current rule-flow blockers are clear.
