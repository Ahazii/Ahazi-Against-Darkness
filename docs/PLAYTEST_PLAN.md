...
5. Run **Dark Plague** once. Confirm that the Narrative names the exposed hero, roll, target level, result, resulting state, and later room-entry effect. A successful save, Blessing cure, or Elven Bread cure grants **Dark Plague immunity** for the current adventure only; it clears during closeout. The pending p.59-vs-p.37 entry-save clarification is noted in `STATUS.md` before treating this as final signoff.
