...
Last updated: 2026-07-15. Target build: post-v0.39.17 FD spider reaction / Citadel return-marker / Greater Mutated Goblin full-rule follow-up patch.
...
The latest disposable Forsaken Depths Game 2 report was session `f0ab5c80eee34a9c9cec7b3e95484823`. Read-only DB inspection showed it was back on main-map **Forsaken Depths Tile 41** while stale Citadel state still said active. The older live session `fed118f1c70a4835b84f245bf8ddefeb` may now be complete. Use the active disposable FD session if it is still available; otherwise start a fresh disposable FD Game 2 after deployment.
...
2. Resume session `f0ab5c80eee34a9c9cec7b3e95484823` if it is still available. It should open on main-map **Forsaken Depths Tile 41** with no `Citadel: ...` marker. If the marker persists, copy a Narrative Report immediately before moving.
3. Force **FD foe encounter -> Vermin -> Spore Spiders**. Confirm the log uses the **Spore Spiders reaction table**, they always fight to the death, and they do not flee on morale. Also note whether the encounter-start 4-in-6 surprise roll appears.
4. Force **FD foe encounter -> Vermin -> Deep Cave Spiders** only if you want one more spider check. Confirm Morale is at `-1`, no treasure is awarded, and if at least two spiders are killed while others remain, the log reduces the remaining spiders by 1 Level per two killed to minimum L3. The printed death-spawn/no-resurrection rule is still a future fidelity hook unless the log explicitly shows it.
5. Force or naturally encounter **Greater Mutated Goblin** once if convenient. Confirm the opening corrosive mucus targets one random hero with a Defense roll vs the goblin's L+1; on failure that hero cannot flee, loses 1 Life per turn, and has -2 Defense until Blessing or goblin defeat. Its normal hit should inflict Tier damage. After defeating it, claim the treasure and confirm the app stages 2d6 small gems worth 25gp each, 5d6gp, and one masterwork edged weapon worth `(d6+4)x5`gp. If the party includes a goblin PC, its reaction roll should show the printed -1 modifier.
6. Continue the Citadel of Traps until a room-content result that would be Minions or Hordes appears. FD p.60 then makes a 4-in-6 replacement check; if it becomes a trap, confirm the room has a concrete FD trap name/level and the **Resolve Trap** action appears. Resolve one such trap and confirm any post-trap treasure can be claimed normally.
7. Move one room farther into a Citadel, save, return to Dashboard, resume, and confirm the same Citadel room restores. Then use the side-sheet return route when available and confirm the Citadel marker clears immediately after returning to the main dungeon map.
8. Force **FD foe encounter -> Boss -> Dark Elf Warlock** if not already reconfirmed after the latest deploy. Confirm the defeated Warlock awards one FD treasure roll at `+2`. Treat the printed ice-blast targeting as still under review unless the log explicitly shows the Defense roll: FD p.44 says the Warlock makes one staff attack and one ice blast each turn; shields count, armor is ignored except Cold defense, and barbarians/Ice-based characters add +1/2 L.
9. During exploration, resolve one FD hazard/event choice and, if offered, accept Lady in Gray or Cyclopean Idol and confirm Ongoing Quests shows the progress condition and one reward or choice path.
10. The following Game 2 checks are complete unless later regression reopens them: FD developer controls appeared with `Supplements: 3`; Shadowbats of the Deep combat looked correct; Infallible Missile created the Level 8+ second missile correctly; Deep Troll chip/hover and Hack button appeared; Deep Troll slain-body return fired before hacking; Deep Troll hack blocked the next group return after the follow-up patch; Deep Troll treasure at `-1` and ordinary minor XP wording were visible; Horde of Dark Elves opening volley and treasure roll were visible; one Boss and one ordinary Weird entered ordinary combat; Undead Leviathan generated FD treasure; FD p.62 treasure choice buttons appeared in the main action strip/Current Objective and accepted schema payloads; forcing Citadel no longer completes the adventure via the dungeon exit; Citadel of Traps creates/reveals only the first side-sheet room on entry; Citadel save/reload restored the same side-sheet room; Spore/Deep Cave/Shadowbat vermin no-treasure outcomes are correct.
