...
Last updated: 2026-07-17. Target build: v0.39.30 TAG rumor/scene text split.
...
1. Deploy v0.39.30 or later and force-refresh until the app header shows that build.
...
3. Confirm the generated lead uses the local PDF scene text, not generic filler: the opening should frame Bofto as a rumour with **Choose to investigate** / **Don't investigate** buttons, Scene 9 should show only the star-object scene prose, and the current-room buttons should include the extracted steal / family / leave choices without exposing `go to Scene` routing.
...
6. If Bofto passes, run one small non-Rumor generated lead smoke check only if time allows: generate either one Treasure Map or one Guild Job and confirm its first prompt/action wording is understandable. Do not restart broad FD/adventure testing unless this v0.39.30 TAG repair fails.
...
Enable **TAG fixed-result controls** immediately before this game. v0.39.30 focuses on Rumor 1 because the previous runs proved the app had the extracted Scene text but first hid the branch buttons/completed too early, then generated scene exits that did not match native tile portals, then exposed PDF routing text directly to the player.
