...
2. In the resumed Citadel room, confirm **Return to main map** is visible even while the current `fd_obsidian_disk` trap is unresolved. It may appear as an exit label, a room-panel button, or a secondary Current Objective action beside **Resolve Trap**. Use it when safe; on the origin/main-map tile, confirm the route back is labelled **Enter Citadel sheet**. Re-enter and confirm the Citadel marker returns only while actually on the side sheet.
