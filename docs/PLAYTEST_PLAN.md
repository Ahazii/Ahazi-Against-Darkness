...
Last updated: 2026-07-19. Target build: v0.39.34 TAG Bofto theft chooser guard.
...
1. Deploy v0.39.34 or later and force-refresh until the app header shows that build.
...
4. Click the Scene 14 theft branch. Confirm both the narrative action strip/Current Objective area and the room prompt give a character dropdown plus **Roll theft Save**, not the old **Steal star object** shortcut and not **If you fail** / **If you succeed** buttons. Choose a thief and confirm the app rolls Scene 14's thievery Save vs L6 with the printed modifier: rogues/halflings +Level, swashbucklers/assassins +half Level, elves +1. Success must move to Scene 19; failure must move to Scene 18.
5. If the theft fails and opens Scene 18, confirm the scene shows only the failure/outcome text and a leave/resolve-style action. It must not show **Record Bofto choice**, **Steal star object**, **Star Will save**, or **Star-Slayer check**.
6. Also click the Scene 17 branch in a fresh or repeated Bofto run if needed. Confirm the party moves to a real **Scene 17** room/prompt with the PDF-derived family text, no visible `playing Scene 9` routing instruction, and only **Insist on investigating** / **You choose to leave** action buttons. **Insist on investigating** must return to Scene 9; **You choose to leave** ends the module.
7. Complete generated-lead closeout and confirm the signoff panel records route/reward/XP/Guild/banking review without leaving a resumable active session.
8. If Bofto passes, run one small non-Rumor generated lead smoke check only if time allows: generate either one Treasure Map or one Guild Job and confirm its first prompt/action wording is understandable. Do not restart broad FD/adventure testing unless this v0.39.34 TAG repair fails.
9. After the TAG scene-chain check passes, stop adventure regression testing for now and resume modularisation only in small tested slices.
...
Enable **TAG fixed-result controls** immediately before this game. v0.39.34 focuses on Rumor 1 because the previous runs proved the app had the extracted Scene text but first hid the branch buttons/completed too early, then generated scene exits that did not match native tile portals, exposed PDF routing/debug text directly to the player, treated Scene 14 success/failure outcomes as player choices instead of rolling the printed Save, and then exposed an old **Steal star object** shortcut that rolled d6+0 without a chosen thief.
...
2. Follow Scene 14 from the visible branch button. Confirm the target scene asks who steals the object, rolls automatically with that character's modifier, and opens Scene 18 or Scene 19 from the result. A no-character d6+0 theft roll must not be possible.
3. If Scene 18 appears, confirm old Bofto procedure buttons are absent there.
4. Follow Scene 17 from the visible branch button. Confirm the target scene room/prompt uses the local PDF-derived text and offers only **Insist on investigating** and **You choose to leave**.
5. Resolve/close out from the scene prompt. Confirm route marker, reward/XP/Guild/banking signoff, and dashboard return work, and no resumable active session remains after closeout.
