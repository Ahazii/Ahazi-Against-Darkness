...
- `src/app/engine/treasure_claims.py` - guarded current-tile treasure claim transaction, including claim-state updates, Old School tally credit, inventory milestones, and imported trigger dispatch
- `src/app/engine/tier_skills.py` - tier-entry validation/payment, prerequisites/costs, tier-skill eligibility, XP-spend fork validation, and tier-skill learning
...
- `src/app/engine/quest_rewards.py` - Lady in White Quest acceptance/refusal, peaceful-progress tracking, core turn-in validation, Epic Reward table claim, payment, inventory gates, and reward state effects
...
- `src/app/engine/combat_lifecycle.py` - session-level combat result merging and reaction-derived first-round state consumption
...
- `src/app/engine/tag_daroc.py` - typed TAG pp.20, 24, and 26 Daroc procedure: selected-searcher Streetwise progress, town-Clue provenance/eligibility, exact Clue spending, selected reward recipient, and 200gp plus XP resolution
- `src/app/engine/tag_mutant_fish.py` - typed TAG p.29 Scene 12 party hypnosis, rescue turns, ration disposition, and campaign-aware sale pricing
- `src/app/engine/tag_scene_lifecycle.py` - declarative generated-TAG room action lifecycle registry for idempotent automatic entry procedures, required-resolution metadata, and canonical shared terminal-closeout guidance
...
- Generated TAG scene completion uses persisted `SessionState.tag_generated_completion_*` fields. A resolved result remains in Narrative through save/resume and blocks unrelated exploration actions until the player chooses **Continue — return to town and finish**. Frontend objectives mark this as a required action, so optional Objective Details preferences cannot hide its primary control; resume normalization changes only stale wording.
...
- `src/app/engine/clues.py` - held-Clue ownership, grants, preferred-holder and party spending, legacy migration, and session-total synchronization
