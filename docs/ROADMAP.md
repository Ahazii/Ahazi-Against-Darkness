...
- **TAG campaign downtime phase (2026-06-28):** troupe name/active-party/guild-coffer state, settlement storage, fixed service purchases, PP tracking, magic locker creation/summoning, Gambling House gp outcomes, Streetwise rumors/interrogation/Look Tough, and treasure-map follow summaries are implemented with home-panel UI and hover hints. Next TAG targets are playable Rumor Scenes, Thematic Dungeons, Guild jobs, solo missions, trinkets, and Guild spells.
- **TAG adventure lead phase (2026-06-28):** Rumor Scenes, Treasure Map leads, Thematic Dungeons, and Guild Jobs can be converted from the TAG settlement panel into normal installed adventure modules under the existing Adventure section/dropdown. Next TAG targets are exact scene branches, exact thematic dungeon table payloads, solo missions, trinkets, and Guild spells.
