...
- Current TAG conversion slice: v0.39.49 makes the primary Current Objective strip reuse Rumor 2's typed Scene 10/1 controls, so the persisted failed group Stealth stage exposes the assassin response instead of an inert generic prompt button. Complete only the stored four-agent decision/routing check in `PLAYTEST_PLAN.md`; do not reroll the approach. The next bounded slice is TAG pp.25-26 Scene 1/6: exact surprise-before-gaze timing, standard Medusa reaction outcomes, 260gp pendant plus 2d6 necros, try-on choice, and rechargeable pendant Luck. Do not build a generic prose interpreter or expose printed success/failure outcomes as player choices.
