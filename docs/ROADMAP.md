...
- Completed TAG Rumor slice: v0.39.56 converts Daroc's Lost Familiar (TAG p.27, Scene 5) into a typed procedure. Only Clues generated in town by Streetwise count; the app persists that provenance, applies the printed one-Clue discount for a Druid, Beastmaster, cat-like hero, or cat animal companion, spends the eligible Clues, awards 100gp to the selected hero, and adds exactly one pending XP roll. Beastmaster and animal-companion mechanics remain deferred to their full Crucible of Classic Critters implementation; the Scene 5 eligibility check already consumes shared class/trait/companion metadata.
