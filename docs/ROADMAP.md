...
- Current TAG conversion slice: v0.39.50 clarifies Rumor 2 Scene 10's TAG pp.6-8 one-roll/lowest-modifier rule, lists every party modifier, and separates its labelled Streetwise actor choice from immediate combat. Complete only the stored four-agent decision/routing check in `PLAYTEST_PLAN.md`; do not reroll the approach. The next bounded slice is TAG pp.25-26 Scene 1/6: exact surprise-before-gaze timing, standard Medusa reaction outcomes, 260gp pendant plus 2d6 necros, try-on choice, and rechargeable pendant Luck. Do not build a generic prose interpreter or expose printed success/failure outcomes as player choices.
