...
- [x] Normalize new/resumed Rumor 9 Scene 5 narrative to the player-confirmed 200gp ruling and make Narrative Debug Reports distinguish carried, banked, and total hero gold (v0.39.67).
- [ ] Finish replacing arrival-based completion across the other generated Rumours. The pp.22-31 audit confirms all twelve require a terminal result. Add shared repeatable vendor/service hosting plus an explicit Done action for Rumors 6/11 next; then implement the mandatory Rumor 3 ambush procedure, Rumor 10's dynamic gargoyle flow, Rumor 8's two-route child dungeon, Rumors 5/7/12 parent-child handoffs, and Medusa last. Do not switch a profile to `tag_scene_resolved` until every printed terminal route can call the shared closeout.
...
- Completed Daroc presentation follow-up: v0.39.67 removes the stale 100gp Scene 5 sentence from new and resumed runtime narrative while leaving the actual 200gp reward unchanged. Narrative Debug Reports now expose carried, banked, and total gold separately, preventing the dungeon carry cap from hiding bribe deductions during diagnosis.
