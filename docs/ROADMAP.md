...
- **TAG route/XP/action phase (2026-06-29):** TAG Actions now handles exact route markers, Clue gates, printed scene rewards, XP markers/awards, per-character bank ledgers, safe generated-module route rewrites, Guild marker clearing, and safe Guild spell state effects with hover-hinted UI. The TAG guide and player-actionable checking docs live under `docs/Checking/`. Next TAG targets are deeper automatic scene insertion/removal and exact combat/exploration modifiers for Guild spell markers.
