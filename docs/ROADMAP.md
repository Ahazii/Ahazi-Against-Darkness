...
- **TAG route/XP/action phase (2026-06-29):** TAG Actions now handles exact route markers, Clue gates, printed scene rewards, XP markers/awards, per-character bank ledgers, and safe Guild spell state effects with hover-hinted UI. Next TAG targets are automatic module rewrites from route markers and exact combat/exploration modifiers for Guild spell markers.
