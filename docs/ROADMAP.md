...
- Current TAG conversion slice: v0.39.51 repairs the persisted Rumor 2 Scene 1 `d6=2 quest` result, presents the printed Accept/Refuse decision without rerolling, and routes acceptance through the Expanded Edition p.162 Quest Table. Complete only its compact-UI and Quest-persistence check in `PLAYTEST_PLAN.md`. The next bounded slice is the remaining TAG pp.25-26 Scene 1/6 reactions and rewards: bribe payment, fight/fight-to-the-death, 260gp pendant plus 2d6 necros, try-on choice, and rechargeable pendant Luck. Do not build a generic prose interpreter or expose printed success/failure outcomes as player choices.
