...
- Current TAG conversion slice: Rumor 2 Scene 10 is implemented in v0.39.47 from TAG pp.6-8 and p.28. It owns the worst-modifier group Stealth Save, persisted d3+2 assassin choice, actor-selected Streetwise Save, printed initiative, and 4d6 gp. Complete the one short UI/save-resume check in `PLAYTEST_PLAN.md`, then select the next PDF-backed scene. Bofto Scene 14 and Medusa Scene 10 are the reference implementations; do not build a generic prose interpreter or expose printed success/failure outcomes as player choices.
