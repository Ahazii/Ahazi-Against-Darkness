...
- [x] Clarify the existing Deoldyn payment contract as each trainee's own carried + home-bank total with bank-first spending, expose that three-part balance in the training UI, and add a saved second horizontal divider between Narrative and the generated service. The inner split defaults to 40/60, preserves at least 96px for Narrative, gives both regions independent scrollbars, and leaves the outer map, Exits, and side-rail resizers independent (v0.39.72).
- [ ] Force-refresh and resume preserved session `fc741849402d46e096b2efa52368de8f` on v0.39.72 while it is still camped. Retry **Return to dungeon** once, follow the explored route to Deoldyn if needed, confirm Sir Benedict shows 0gp carried + 600gp banked = 600gp available, exercise and reset both horizontal dividers, then run his bank-funded automatic training result, Done, and Continue. The opening, transfers, earlier Scene 3 entry/outer-divider check, and Rumor 6 already passed; do not replay them.
