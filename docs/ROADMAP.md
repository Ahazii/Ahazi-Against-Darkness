...
- Current TAG conversion slice: v0.39.53 closes the accepted Rumor 2 Scene 1 `d6=2 quest` route after its core Expanded Edition Quest is completed. The generated lead now resolves peacefully, with no combat treasure, and offers a direct return-to-town closeout; already-completed saves repair on resume without rerolling rewards. Complete only that resume/closeout check in `PLAYTEST_PLAN.md`. The next bounded slice is the remaining TAG pp.25-26 Scene 1/6 reactions and rewards: bribe payment, fight/fight-to-the-death, 260gp pendant plus 2d6 necros, try-on choice, and rechargeable pendant Luck. Do not build a generic prose interpreter or expose printed success/failure outcomes as player choices.
