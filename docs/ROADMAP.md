...
- Current TAG conversion slice: reuse `tag_scene_actions.py` for one PDF-backed scene procedure, including required actor, modifiers, target, attempts, outcomes, and rewards. Bofto Scene 14 is the reference implementation. Inspect the selected scene in the owned PDF before coding; do not build a generic prose interpreter or expose printed success/failure outcomes as player choices.
