...
- Current TAG conversion slice: v0.39.48 repairs Rumor 2's local scene graph by PDF scene identity, so resumed Scene 10 uses its typed worst-modifier group Stealth procedure instead of extracted sentence fragments and routes explicitly to Scene 1 or town. Complete the short resume/routing check in `PLAYTEST_PLAN.md`. The next bounded slice is TAG pp.25-26 Scene 1/6: exact surprise-before-gaze timing, standard Medusa reaction outcomes, 260gp pendant plus 2d6 necros, try-on choice, and rechargeable pendant Luck. Do not build a generic prose interpreter or expose printed success/failure outcomes as player choices.
