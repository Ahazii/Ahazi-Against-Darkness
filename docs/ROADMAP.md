...
- [x] Make the shared TAG Rumor opening visible for every generated quest subtype, centralize all three frontend special-action dispatch paths, use **Not now — return to town**, and give every Rumor an explicit first Scene; legacy Bofto modules now enter Scene 9 (v0.39.63).
- [x] Share the TAG pp.22-24 investigate-now/retain-for-later prompt metadata across all twelve generated Rumors, including existing-save upgrades (v0.39.62).
- [x] Add reusable `auto_start` / `required_for_completion` TAG scene metadata, an idempotent room-entry action registry, `tag_scene_resolved` gating, terminal-failure persistence, and legacy-session repair; migrate Rumor 4 so Scene 12 automatically rolls every living hero's hypnosis Save and cannot complete on arrival (v0.39.64).
- [x] Make every pending generated-TAG terminal acknowledgement a required, non-hideable primary action; align Narrative, UI, diagnostics, resume repair, and Tables on **Continue — return to town and finish** (v0.39.65).
- [x] Migrate Rumor 9 to required scene completion with repeated actor-selected Streetwise searches, persisted Town Clue progress, a non-permanent Give up route, the 200gp Rumor offer, and one XP roll (v0.39.66).
- [x] Normalize new/resumed Rumor 9 Scene 5 narrative to the player-confirmed 200gp ruling, make Narrative Debug Reports distinguish carried, banked, and total hero gold, and pass the bounded non-mutating presentation gate (v0.39.67).
- [x] Add one shared persisted vendor/trainer service host plus an explicit required Done action for Rumor 6's TAG p.23 / Scene 2 pp.25-26 Blackbird Hill bargain and Rumor 11's TAG p.24 / Scene 3 p.26 Deoldyn training. Fresh and legacy manifests use `tag_scene_resolved`; Rumor 6 purchases may repeat until Done, while Rumor 11 accepts one complete simultaneous batch and forbids additions after its rolls, without leaking service actions into the shared opening (v0.39.68).
- [x] Correct Rumor 6's Scene 2 lesson eligibility to the player-confirmed campaign interpretation: any living non-Barbarian may learn automatically as from a scroll, with one use per adventure at +1 for a non-spellcaster. Retain a vertical scrollbar at constrained heights so the lesson controls and existing Done action cannot be clipped (v0.39.69). The full Rumor 6 live gate passed on 2026-08-04 and must not be replayed.
- [x] Restore the shared service-host layout contract exposed by Rumor 11: the user's horizontal Narrative/map divider height wins, while an internally scrolling service panel keeps training and Done reachable (v0.39.70).
- [x] Preserve the shared Rumor opening across camp/re-entry: match the exact resolved Investigate marker instead of current-room identity, persist future opening decisions, allow fresh start-camped Rumors to enter before choosing, reuse one server-derived pending flag across Narrative and Exits, and report a meaningful return result (v0.39.71).
- [x] Clarify the existing Deoldyn payment contract as each trainee's own carried + home-bank total with bank-first spending, expose that three-part balance in the training UI, and add a saved second horizontal divider between Narrative and the generated service. The inner split defaults to 40/60, preserves at least 96px for Narrative, gives both regions independent scrollbars, and leaves the outer map, Exits, and side-rail resizers independent. Keep that inner divider visible whenever the tall guided service is visibly active, including when Objective Details is collapsed (v0.39.73).
- [ ] Force-refresh the preserved Rumor 11 session `fc741849402d46e096b2efa52368de8f` on v0.39.73, confirm the inner divider is visible with Objective Details collapsed, exercise/reset both horizontal dividers, then run Sir Benedict's bank-funded automatic training result, Done, and Continue. The opening, transfers, return, earlier Scene 3 entry/outer-divider check, and Rumor 6 already passed; do not replay them.
- [ ] Finish replacing arrival-based completion across the other generated Rumours. The pp.22-31 audit confirms all twelve require a terminal result. After the remaining bounded Rumor 11 live gate, implement the mandatory Rumor 3 ambush procedure, Rumor 10's dynamic gargoyle flow, Rumor 8's two-route child dungeon, Rumors 5/7/12 parent-child handoffs, and Medusa last. Do not switch a profile to `tag_scene_resolved` until every printed terminal route can call the shared closeout.
...
- Completed TAG conversion slice: v0.39.54 completes Rumor 2 Scene 1/6. The accepted `d6=2 quest` route already passed live; focused automation owns the persisted `6d6` gold-or-gem bribe, refusal/combat, fight-to-the-death Morale state, one major-foe XP roll, `2d6` necros, wear-or-sell pendant choice, and rechargeable one-point/two-point halfling Luck allowance. Do not reopen a full Rumor 2 playthrough.
- Completed TAG workflow repair: v0.39.60 makes TAG p.28 Scene 10's **Approach the cabin** / **Return to town** choices horizontal and routes return-to-town through normal adventure completion, summary, roster/campaign persistence, and automatic homecoming deliveries. The repair recognizes the already-clicked live Rumor 2 state.
- Completed TAG spell follow-up: v0.39.55 replaces Temporary Weapon Enchantment's manual Guild-marker expiry with TAG p.65's exact seven-day campaign clock and qualifying magic-only encounter timing. The selected weapon is magical without an Attack bonus; an attack attempt against a strictly magic-only foe schedules encounter-end expiry, while the tested theft/destruction decision remains shared encounter policy.
- Completed TAG Rumor slice: v0.39.56 introduced Daroc's typed Town Clue spend/reward resolver; v0.39.66 completes its generated-adventure lifecycle. TAG p.20 supplies an actor-selected, repeatable L6 Streetwise search with a `d6` bribe; TAG p.26 supplies the two-Clue requirement and one-Clue Druid/Beastmaster/cat exception. **Give up** returns the Rumor to `heard` without discarding eligible Town Clues. Success spends those Clues, awards the selected living hero the player-confirmed 200gp offer from TAG p.24 plus one pending XP roll, and prevents arrival-based or duplicate completion. Scene 5's conflicting 100gp line on p.26 is treated as the known source error. Full Beastmaster and animal-companion mechanics remain deferred to their Crucible of Classic Critters implementation.
- Completed Rumor UI regression: v0.39.57 keeps generated Rumor entry branch buttons directly beneath Narrative and removes Director phase/playbook prose from the player decision surface. The choices remain visible when Objective Details is hidden, and later typed scene controls retain the same Narrative ownership.
- Completed Rumor choice layout follow-up: v0.39.58 keeps multiple generated Rumor entry choices side by side at normal app widths, with wrapping reserved for narrow mobile layouts.
- Completed TAG Rumor slice: v0.39.59 converts Mutant Fish Under the Bridge (TAG p.29, Scene 12) from three manual outcome controls into one typed, persistent hypnosis/rescue/reward procedure. It owns all party Saves, rescue-turn Life loss and role swaps, total-party destruction, `d6+3` rations, the campaign-scoped chaos-cultist sale rate, and two-minion XP progress.
- Completed TAG lifecycle correction: v0.39.64 automatically starts Rumor 4's per-character Saves on Scene 12 entry, defers completion through rescue and Keep/Sell, repairs already arrival-completed saves without rerolling, and reuses one declarative lifecycle registry for future typed Rumour terminals.
- Completed generated-TAG closeout correction: v0.39.65 keeps the terminal action prominent despite collapsed Objective Details, uses one canonical label across all shared closeout surfaces, and normalizes existing pending sessions without replaying their scenes.
- Completed generated-TAG Daroc lifecycle: v0.39.66 keeps search progress inside the shared scene host, preserves it across save/resume and non-permanent Give up, and permits pending XP assignment/banking before the canonical Continue closeout.
- Completed Daroc presentation follow-up: v0.39.67 removes the stale 100gp Scene 5 sentence from new and resumed runtime narrative while leaving the actual 200gp reward unchanged. Narrative Debug Reports now expose carried, banked, and total gold separately, preventing the dungeon carry cap from hiding bribe deductions during diagnosis; the live presentation gate passed.
- Completed shared repeatable-service implementation: v0.39.68 makes Rumor 6's Blackbird Hill purchases and Rumor 11's Deoldyn training reuse one persisted transaction/Done lifecycle and one guided UI rather than separate arrival-complete modules. The player-confirmed Shoes interpretation allows living eligible heroes and living hirelings, excludes animal companions, uses party Tier for a hireling wearer, retains party ownership, and returns the pair when that hireling leaves. Deoldyn validates all selected bow-capable trainees against their own carried + home-bank funds, deducts every payment bank-first, and then rolls the batch; only the normal/base Elf receives the ordinary-level option. Rumor 6 is live-passed; Rumor 11's opening, initial Scene 3 entry, outer-divider correction, transfers, camp return, and route back to Deoldyn are live-passed. Its remaining v0.39.73 gate is the visible inner divider with Objective Details collapsed, one bank-funded training batch, Done, and Continue.
- Live-state follow-up: Rumor 11's return retry and route back to Deoldyn subsequently passed. The preserved save is inside the adventure at an open unresolved service; only the v0.39.73 collapsed-Objective-Details divider confirmation, training batch, Done, and Continue remain.
- Completed Blackbird Hill correction implementation: v0.39.69 preserves the three purchases already recorded by v0.39.68, replaces the overly narrow class-list lesson filter with the player-confirmed any-living-non-Barbarian interpretation, gives a non-spellcaster one use per adventure at +1, and keeps service controls reachable through internal scrolling. The 2026-08-04 live follow-up passed the free lesson and closeout; do not replay Rumor 6. v0.39.70 removes the service height floor that Rumor 11 proved could defeat the user's shared map divider.
- Rumor 3 Scene 11 is intentionally deferred as a larger slice because its branch enters the full TAG pp.34-35 Riff-Raff and Outside of Town Ambush tables. Do not approximate those encounters as a small prompt conversion.
...
- Go Adventure now uses Start, Resume, Generate, Guild Jobs, and Reference tabs. Exploration now calls the play text **Narrative** and lets the player show/hide Current Objective, Text Commands, Exits, and Character Sheets. Developer-only controls use one bounded movable, non-closable window whenever any developer preference is active; player controls remain docked.
