...
- Current TAG conversion slice: v0.39.52 completes the accepted Rumor 2 Scene 1 `d6=2 quest` route. A core Expanded Edition p.162 Bring Gold Quest now remains a core Quest inside the imported TAG session, exposes its exact stored amount and Quest-giver turn-in, deducts that amount, and awards one Epic Reward. Complete only its turn-in and floating Developer Options check in `PLAYTEST_PLAN.md`. The next bounded slice is the remaining TAG pp.25-26 Scene 1/6 reactions and rewards: bribe payment, fight/fight-to-the-death, 260gp pendant plus 2d6 necros, try-on choice, and rechargeable pendant Luck. Do not build a generic prose interpreter or expose printed success/failure outcomes as player choices.
