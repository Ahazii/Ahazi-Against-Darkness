...
- [x] Correct Rumor 6's Scene 2 lesson eligibility to the player-confirmed campaign interpretation: any living non-Barbarian may learn automatically as from a scroll, with one use per adventure at +1 for a non-spellcaster. Auto-expand the Blackbird Hill guided service and retain a vertical scrollbar at constrained heights so the lesson controls and existing Done action cannot be clipped (v0.39.69).
- [ ] Finish replacing arrival-based completion across the other generated Rumours. The pp.22-31 audit confirms all twelve require a terminal result. After the saved Rumor 6 free-lesson/Done check and the bounded Rumor 11 live gate, implement the mandatory Rumor 3 ambush procedure, Rumor 10's dynamic gargoyle flow, Rumor 8's two-route child dungeon, Rumors 5/7/12 parent-child handoffs, and Medusa last. Do not switch a profile to `tag_scene_resolved` until every printed terminal route can call the shared closeout.
