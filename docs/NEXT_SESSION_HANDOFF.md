...
- Resume Forsaken Depths Game 2 session `f0ab5c80eee34a9c9cec7b3e95484823` if still active, or start a fresh disposable FD Game 2. First confirm the stale Citadel marker is gone when the party is on main-map Forsaken Depths Tile 41. Then force **FD foe encounter -> Vermin -> Spore Spiders** once to confirm the named reaction table, fight-to-death, no morale flee, and 4-in-6 surprise logging. If convenient, defeat **Greater Mutated Goblin** once and confirm FD p.41 fixed loot stages. Then force/continue **Citadel of Traps** until a Minions/Hordes room-content result is eligible for the FD p.60 trap-replacement check and resolve one concrete trap if it appears.
