...
- Resume Forsaken Depths Game 2 if still active. First force **FD foe encounter -> Minions -> Deep Trolls** once to confirm slain-troll return, FD treasure at `-1`, active FD debug context, and ordinary minor XP wording. Then select **FD foe encounter -> Citadel Weird** in the named foe dropdown for the next forced foe.
