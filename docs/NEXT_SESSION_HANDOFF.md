...
Last updated: 2026-07-16. Repository branch: `main`. Latest release: `v0.39.17` (`961dae0`).
...
- Resume disposable session `003f51881ebc417a8a70891da9e0de00` if still active, or start a fresh EE+Abyss+FD Game 2. First confirm the current Citadel room exposes **Return to main map**, the origin exposes **Enter Citadel sheet**, and re-entry restores the active Citadel marker only while actually on the side sheet. If Sly Silas or another hero still has `FD Soulbound:<tile id>`, cast **Blessing** on that hero and confirm the cure log plus cleared chip/pending choice. Do not repeat Spore Spiders, Deep Cave Spiders, Greater Mutated Goblin, or stale Citadel badge checks unless a later regression reopens them. The latest Citadel of Traps evidence already showed a Minions/Hordes replacement created a concrete active trap with **Resolve Trap**; next resolve that trap and continue the remaining Citadel save/return checks.
