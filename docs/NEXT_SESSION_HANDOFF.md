...
- Resume Forsaken Depths Game 2 session `f0ab5c80eee34a9c9cec7b3e95484823` if still active, or start a fresh disposable FD Game 2. First confirm the stale Citadel marker is gone when the party is on main-map Forsaken Depths Tile 41 and Copy Narrative Report says `Citadel: none` with `Side sheet: none; active=false; origin=none`. Do not repeat Spore Spiders, Deep Cave Spiders, or Greater Mutated Goblin unless a later regression reopens them; the latest report already gives pass evidence for their current patched behavior. Then force/continue **Citadel of Traps** until a Minions/Hordes room-content result is eligible for the FD p.60 trap-replacement check and resolve one concrete trap if it appears.
