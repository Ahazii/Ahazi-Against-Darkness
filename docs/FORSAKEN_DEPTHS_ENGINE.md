...
- Map elements draw from catalog `forsaken_depths` (dungeon) or `forsaken_depths_rivers` (underground river). FD start tiles also use `forsaken_depths`; the engine adds one open **Dungeon Exit** to the start tile if the printed FD map element does not contain an outside exit.
...
| **Soulbinding choices** | Bound hero enters another area | `resolve_fd_soulbinding_choice` (1 Life or 1 Madness per area away; Blessing frees the hero) |
...
|-------------|--------------|
| **Complex Machinery** | Room panel offers **Try for Clue** and **Try for Food** with a living-hero selector. Each hero can attempt once on the first visit; success grants the chosen **1 Clue** or **d6 Food rations**, failure applies Tier damage. The check is **HCL+2**; gnomes/artificers add Level; wizards add half Level. |
...
- **fd_trap** room content seeds an FD trap (`fd_trap_table`, level HCL+Tier+2). **Resolve trap** now follows FD p.58 directly: all trap Saves add half Level, named class/armor modifiers are applied only where the row says so, Halflings get the printed reroll, Beast Cage spawns a no-treasure surprise Weird, Soulbinding marks the hero with a visible Soulbound state chip and creates an area-away Life/Madness choice until Blessing frees that hero, Disintegration pauses for the permanent-magic-item sacrifice choice, and Magic Resistant Liquid blocks spell/prayer/scroll/magic-item casting while its six-room counter remains. Room traps have a 2-in-6 FD treasure roll after clearing.
- **Monster treasure** uses `fd_treasure_table` when `ruleset=forsaken_depths` after combat on slain foes with explicit `treasure_rolls` on the FD bestiary template, or with a printed `treasure_modifier` / `treasure_bonus` that implies one FD treasure roll. Gem/jewelry rows add pocket items as `Gem (Ngp)` rather than raw gold. Rolls with a choice (gold/masterwork, silver weapons/arrows, potions/scrolls, clue scroll/magic item, etc.) show **Treasure** map markers with pick buttons before claim, and Current Objective points to the unresolved treasure choice. Concrete weapon picks are required before claim for FD p.62 masterwork weapons, FD p.62 silvered melee weapon bundles, and the Greater Mutated Goblin's FD p.41 masterwork edged weapon value. Roll **10** (jackpot) offers **roll twice** or **roll four times** (4-in-6 wandering monsters when claiming loot). Vermin without `treasure_rolls` do not default to a roll in FD sessions.
...
- **Earthquake** (`fd_event_table` roll 6): each living hero rolls **d3 falling stones**; each stone is a separate **Save vs HCL**, and each failed Save costs 1 Life.
...
- **TAG settlement Apothecary UI** — when TAG banking/settlement mode is enabled, the session side panel exposes **TAG Settlement Apothecary** separately from **Camp Outside Dungeon**. The action uses `tag_settlement_brew_apothecary` and the TCOTFD recipe engine; a living Wandering Alchemist with mortar and pestle, listed ingredients, and material gp are still required.
...
- **TCOTFD playable classes** — `courtship_classes.py` + `classes.json`: **Wandering Alchemist** (halfling saves, ingredient harvest reroll, Demesne brewing, innate Flower Portal once per adventure); **Satyr** (+L Defense, 2×L woo/mesmerize vs flower demons, auto-fail vs Maidens/Ladies/Matrons, outdoor pheromone seduction, innate Blossoms casts once per level); **Conservationist** (L+3 peaceful spell slots, Blossoms transcription, forbidden harmful spells trigger Curse of Tamas Zeya / BoS entry 16); **Demonologist** (+½L woo, +L Blossoms scroll casts); **Cambion/Succubus** (hidden Riverside pathway with cleric/paladin, +L Madness saves). Halflings may spend Luck on failed Giving/Withholding rolls. TCOTFD trained Surgeon/Herbalist/Poison Expert skills are exposed in the camp/professional UI with hover hints.
...
- **Magic Citadel** final room: clear the Weird guardian with **+1 Life** and altar-spell immunity, then **Interact with Cyclopean Idol**.
...
- **Heroic spell polish** — Teleport Enemy queues the foe in visited-room return state and ticks it room/hex/area by room/hex/area on movement/combat turns, blocking return when the route becomes impassable. If the returning foe crosses an occupied room, the engine now rolls that room's occupant reaction through the existing reaction table/bestiary path and logs whether the occupants engage or let the foe continue. Mass Blessing supports selected party/hireling targets and selected Blessing-removable conditions, with caster Life cost based on the actual selections. Fire of Truth applies its +1 chaos-creature bonus to the actual spell roll and exposes that rule in spell hover text.
