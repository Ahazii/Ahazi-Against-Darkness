...
- **Heroic spell polish** — Teleport Enemy now queues the foe in visited-room return state and ticks it room/hex/area by room/hex/area on movement/combat turns, blocking return when the route becomes impassable. Mass Blessing now supports selected party/hireling targets and selected Blessing-removable conditions, with caster Life cost based on the actual selections. Fire of Truth applies its +1 chaos-creature bonus to the actual spell roll and exposes that rule in spell hover text.
- **Remaining Heroic spell edges** — Teleport Enemy crossing occupied rooms currently logs the reaction opportunity for manual resolution; a future pass can automate the occupant reaction/fight branch.
