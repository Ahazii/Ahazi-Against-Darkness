...
def test_generated_tag_result_waits_for_continue_before_closeout() -> None:
    ...
    assert 'TAG_GENERATED_CLOSEOUT_ACTION_LABEL = "Continue — return to town and finish"' in APP_JS
    assert "requiredAction: true" in APP_JS
    assert 'classList.contains("required-action")' in APP_JS
    assert "panels.objective === false && !requiredObjectiveVisible" in APP_JS
    assert "Pending closeout action:" in APP_JS
...
def test_adventure_pdf_source_scanner_is_exposed_without_marking_pdfs_playable() -> None:
    ...
    assert "Re-extract Maps" in MODERN_PAGES_JS
    assert "Extracted Artwork Library" in MODERN_PAGES_JS
    assert "Extract Artwork Library" in MODERN_PAGES_JS
    assert "/api/adventures/packages/${encodeURIComponent(pkg.package_id)}/extract-artwork" in MODERN_PAGES_JS
    ...
    assert ".modern-package-artwork-grid" in STYLES_CSS
    assert ".modern-package-artwork-frame" in STYLES_CSS
...
def test_modern_home_routes_and_pages_are_standalone() -> None:
    ...
    for label in [
        ...
        "supplementConflictIds",
        "suggestedLegacyProfileForSupplements",
        "profileManuallyChanged",
        "conflicts with selected default",
        "conflicts with selected session supplement",
        ...
        "State diagnostics",
        "modern-state-registry-search",
        "modern-state-registry-family",
        "modern-state-registry-scope",
        "modern-state-registry-supplement",
        "modern-state-registry-review",
        "stateLegacyMappingText",
        "registrySummaryWithHighlight",
        ...
        "modern-terrain-registry-supplement",
        "modern-terrain-registry-review",
        "modern-terrain-registry-interaction",
        "terrainLegacyMappingText",
...
def test_modern_reference_and_table_groups_default_collapsed_and_trim_search() -> None:
    ...
    assert "grid-template-columns: minmax(17rem, min(var(--source-left-width, 26rem), 45%)) 7px minmax(22rem, 1fr)" in STYLES_CSS
    ...
    assert "actionGroup(\"Assign\", [ignorePhraseButton])" in MODERN_PAGES_JS
