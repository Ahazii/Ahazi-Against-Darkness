...
def test_holy_water_hits_and_destroy_skeleton(monkeypatch) -> None:
    ...
    skeleton = _skeleton()
    monkeypatch.setattr("app.engine.consumables.roll_exploding_for_level", lambda *args, **kwargs: (6, [6]))
...
def test_summoned_beast_takes_damage_from_foes(monkeypatch) -> None:
    monkeypatch.setattr("app.engine.random_dungeon.roll_exploding_for_level", lambda *args, **kwargs: (6, [6]))
    eng = engine()
