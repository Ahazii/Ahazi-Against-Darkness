...
def test_acrobat_evade_marks_evasion(monkeypatch) -> None:
    monkeypatch.setattr("app.engine.class_abilities.roll_exploding_d6", lambda: (6, [6]))
...
def test_gnome_trap_disarm_with_bonus(monkeypatch) -> None:
    monkeypatch.setattr("app.engine.class_abilities.roll_exploding_d6", lambda: (4, [4]))
    session = _session()
    gnome = _gnome()
